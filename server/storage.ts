...
const sqlite = new Database("wc2026.db");
...
try { sqlite.exec(`ALTER TABLE users ADD COLUMN email TEXT NOT NULL DEFAULT ''`); } catch {}
...
export interface IStorage {
  ...
  getLeaderboard(): User[];
  addUserPoints(userId: number, delta: number): void;
  ...
  getPredictionsByUser(userId: number): Prediction[];
  getPredictionByUserAndMatch(userId: number, matchId: number): Prediction | undefined;
  updatePrediction(id: number, homeScore: number, awayScore: number): Prediction;

  // Group predictions
  upsertGroupPrediction(data: InsertGroupPrediction): GroupPrediction;
  getGroupPredictionsByUser(userId: number): GroupPrediction[];
  getGroupPredictionByUserAndGroup(userId: number, group: string): GroupPrediction | undefined;
}
...
export const storage: IStorage = {
  ...
  updateUserPassword(userId, newPassword) {
    db.update(users).set({ password: newPassword }).where(eq(users.id, userId)).run();
  },
  getLeaderboard() {
    return db.select().from(users).orderBy(desc(users.points)).all();
  },
  addUserPoints(userId, delta) {
    const user = db.select().from(users).where(eq(users.id, userId)).get();
    if (user) {
      db.update(users).set({ points: user.points + delta }).where(eq(users.id, userId)).run();
    }
  },
  ...
  seedMatches() {
    const existing = db.select().from(matches).get();
    if (existing) return;
  ...
  getPredictionsByUser(userId) {
    return db.select().from(predictions).where(eq(predictions.userId, userId)).all();
  },
  ...
  updatePrediction(id, homeScore, awayScore) {
    return db.update(predictions).set({ homeScore, awayScore }).where(eq(predictions.id, id)).returning().get();
  },

  // Group predictions
  upsertGroupPrediction(data) {
    const existing = db.select().from(groupPredictions).where(
    ...
    return db.insert(groupPredictions).values(data).returning().get();
  },
  getGroupPredictionsByUser(userId) {
    return db.select().from(groupPredictions).where(eq(groupPredictions.userId, userId)).all();
  },
  getGroupPredictionByUserAndGroup(userId, group) {
